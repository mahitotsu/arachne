...
**次の主作業は Phase 5-B に進み、`order-service` と `customer-service` の PostgreSQL 永続化を Spring Data JDBC へ寄せる。その後に Observation を積む。**

次アクション:
1. `order-service` の confirmed orders を Spring Data JDBC aggregate + repository へ移行し、保存・直近注文取得・履歴取得の境界を整理する
2. `customer-service` の customer accounts を Spring Data JDBC aggregate + repository へ移行し、認証 lookup / profile lookup / seed 処理を整理する
...
4. Phase 5-B / 5-C の 1 slice が落ち着いた時点で、あらためて `make up` と E2E 確認を回して最終完了基準も並行して詰める
