...
次アクション:
1. `order-service` の downstream 呼び出しを capability ベースのメタツール委譲へ置き換える境界を定義する
2. `kitchen-service` の `menu_substitution_lookup` を具体サービス名に依存しない委譲パターンへ移行する
3. スキル/プロンプトから具体的なサービス名・エンドポイント記述を外し、registry discover を使う形へ整理する
