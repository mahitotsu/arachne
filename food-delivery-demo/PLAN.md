...
**Phase 3-B（メタツール委譲パターンへの移行）に進む。**

次アクション:
1. `order-service` の `menu` / `delivery` / `support` 委譲を capability ベースのメタツールへ置き換え、service 名や path 前提をプロンプト・スキルから外す
2. `kitchen-service` の `menu_substitution_lookup` を capability ベース委譲へ移し、menu 固定前提をスキルから外す
3. 2-C で整えた gateway / resolver / test-support パターンを前提に、3-B の変更を各 service の境界内へ閉じ込める
