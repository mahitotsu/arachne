...
**Phase 1-B（delivery-agent 強化）に進む。**

次アクション:
1. delivery-service に `discover_eta_services` と `call_eta_service` メタツールを追加し、registry-service の discover 契約を実利用に接続する
2. 自社配送・Hermes・Idaten の ETA/料金/混雑度を文脈付きで比較し、推奨 tier と理由を返す ranking ロジックを固める
3. `急いで` / `安く` などの文脈で推奨が変わる回帰テストを `delivery-service` に追加する
