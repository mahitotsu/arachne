...
**Phase 2-A（registry-service + 外部 ETA モック追加）に進む。**

次アクション:
1. registry-service の API 契約（register / discover / services / health）を固定し、各サービスが起動時登録できる最小実装を入れる
2. hermes-adapter / idaten-adapter / icarus-adapter の登録モデルを追加し、discover で NOT_AVAILABLE を除外する基盤を作る
3. delivery-agent 強化で利用する `discover_eta_services` / `call_eta_service` の境界を Phase 2-A の契約上で先に固定する
4. customer-ui の trace panel は Phase 4-C まで維持し、UI 改修に着手するまで削除しない
