...
**完了基準:**
- [ ] `order-service` / `support-service` / `menu-service` / `kitchen-service` の registry resolver が `*Application.java` 外へ分離されている
- [ ] `order-service` と `support-service` の `Application Service` / downstream `Gateway or Client` が `*Application.java` 外へ分離され、interface 境界が導入されている
- [ ] `order-service` の主要な変更点が単一巨大ファイルに集中しない構成になっている
- [ ] registry lookup 用の test helper 重複が減り、各 API test が service 固有の差分に集中している
- [ ] 3-B 前に必要なサービスは Spring の典型的な責務分割へ寄っており、残りサービスへ同パターンを横展開できる状態になっている
...
**完了基準:**
- [ ] `delivery-service` / `payment-service` / `customer-service` が 2-C と同じ責務構造へ移行している
- [ ] 境界インターフェースと実装の分離が残りサービスにも適用されている
- [ ] test support の重複がさらに減り、残りサービスの API test が service 固有の差分に集中している
- [ ] `food-delivery-demo` 全体で責務分割と visibility の方針が揃っている
- [ ] `mvn -f food-delivery-demo/pom.xml test` が通る
...
| Phase 2-C: Spring ベストプラクティスへの責務分割と境界インターフェース導入 | 🔲 未着手 |
| Phase 3-A: URL解決 registry 経由 | ✅ 完了 |
| Phase 3-B: メタツール委譲 | 🔲 未着手 |
| Phase 3-C: 残りサービスへの構造標準化の横展開 | 🔲 未着手 |
...
**Phase 2-C（Spring ベストプラクティスへの責務分割と境界インターフェース導入）に進む。**

次アクション:
1. `order-service` の `Application Service` / downstream `Gateway` / registry resolver を interface 境界つきで `*Application.java` から分離する
2. `support-service` の gateway / resolver を `order-service` と同じ責務構造へ寄せる
3. `menu-service` / `kitchen-service` の resolver と gateway を独立ファイルへ切り出し、3-B で触る境界を先に整理する
4. registry lookup 回帰で重複している test helper を test support へ寄せ、残りサービスへ横展開できる足場を作る
