...
- [x] Arachne の `AgentResult.metrics()` と Spring の observation-only event bridge を起点に、主要サービスで agent/tool 呼び出しの観測が Actuator / Micrometer から確認できる（対応: Phase 5-C）
...
**変更内容:**
- 各サービスの downstream 設定から直接エンドポイント URL を削除し、registry-service 経由の解決に寄せる
- 最終形では downstream collaborator 解決の入力を `service-name` ではなく `capability-query` に統一し、`POST /registry/discover` で URL と `requestPath` を解決する
- `GET /registry/services` は全登録サービスの一覧・仕様表示用に残し、サービス間 collaborator 解決には使わない
- compose.yml の環境変数を整理

**進捗メモ（2026-04-30 時点の最終状態）:**
- `order-service` / `menu-service` / `kitchen-service` / `support-service` の downstream collaborator 解決は、すべて `capability-query` + `POST /registry/discover` に統一済み
- 4 サービスとも downstream の `service-name` と direct `base-url` fallback 設定を除去し、registry が返す `endpoint` と `requestPath` をそのまま使う
- `compose.yml` から該当 downstream edge 用の `*_SERVICE_NAME` 環境変数を削除済み
- `GET /registry/services` は `/agents` の仕様ビューワーや OpenAPI 参照用の一覧 API としてのみ利用する
...
- 2026-04-30 の follow-up で `menu-service` と `support-service` も同じ discover-only パターンへ揃え、Phase 3-B の downstream collaborator 解決は 4 サービス横断で完了
...
**完了基準:**
- [x] 主要サービスで agent invocation count / latency が Actuator 経由で確認できる
- [x] Bedrock 利用時は token/cache usage が `AgentResult.metrics()` 由来で観測できる
- [x] observation listener が control flow を変えず、Arachne の observation-only 境界を破っていない
- [x] `mvn -f food-delivery-demo/pom.xml test` が通る

**進捗メモ（2026-04-28）:**
- `order-service` から着手し、`OrderApplicationService` の workflow entrypoint (`suggest` / `confirm-items` / `confirm-delivery` / `confirm-payment` / `session` / `recent-order-history`) に `ObservationRegistry` ベースの `delivery.order.workflow` timer を追加した
- low-cardinality tag は `operation` / `outcome` に限定し、既存 control flow や API 契約を変えずに count / latency を Micrometer へ流す最小 slice に留めた
- `order-service` の Actuator 公開設定は `metrics` まで広げ、`OrderServiceApiTest` で `/actuator/metrics` と `/actuator/metrics/delivery.order.workflow` から `suggest` / `success` タグ付きメトリクスが見えることを回帰確認した
- `RegistryBackedMenuGateway` / `RegistryBackedDeliveryGateway` / `RegistryBackedPaymentGateway` / `RegistryBackedSupportGateway` に `delivery.order.downstream` timer を追加し、`RegistryServiceEndpointResolver` には `delivery.order.registry.lookup` timer を追加した。tag は同じく `target` / `operation` / `outcome` に限定した
- `menu-service` / `delivery-service` / `support-service` では agent 実行を `delivery.agent.invocation` として観測し、`ArachneLifecycleApplicationEvent` listener から `delivery.agent.model.call` / `delivery.agent.tool.call` を積む構成を追加した。Bedrock 利用時の `AgentResult.metrics().usage()` は `delivery.agent.usage.tokens` / `delivery.agent.usage.cache` に反映される
- `menu-service` には `delivery.menu.downstream` / `delivery.menu.registry.lookup`、`delivery-service` には `delivery.delivery.downstream` / `delivery.delivery.registry.lookup`、`support-service` には `delivery.support.downstream` / `delivery.support.registry.lookup` を追加し、主要 RestClient 境界を service ごとに分離して見られるようにした
- `food-delivery-demo/README.md` と `docs/architecture.md` に、認証付き `actuator/metrics` での multi-service 確認手順と metric 名を反映した
- minimal compose slice (`registry-service`, `postgres`, `redis`, `customer-service`, `kitchen-service`, `menu-service`, `order-service`) で live smoke を実施し、sign-in → `/api/order/suggest` → `delivery.order.workflow` / `delivery.order.downstream` / `delivery.order.registry.lookup` の可視性を確認した。registry metric は起動順や resolver cache の影響を受けるため、README の確認手順は base metric query を見る形に寄せた
- `MenuServiceApiTest` / `DeliveryServiceApiTest` / `SupportServiceApiTest` を service-local metrics 前提へ更新し、最終的に `mvn -f /home/akring/arachne/food-delivery-demo/pom.xml test` が再度通過した
...
| Phase 5-C: Micrometer Observation と Arachne metrics の可視化 | ✅ 完了 |

## Next Action

**次の主作業は、未チェックの最終完了基準を runtime/E2E で閉じることです。**

次アクション:
1. `make up` で全サービス healthy 起動を再確認する
2. 注文ワークフロー、support handoff、agents viewer を live runtime で通して未チェックの最終完了基準を閉じる
3. registry 経由 URL 解決と skill 記述の acceptance を runtime 目線で再確認する
