...
**Phase 4-C（注文ワークフローUI）に進む。**

次アクション:
1. `order/page.tsx` を4ステップワークフロー（初期入力 → アイテム選択 → 配送選択 → 支払い承認）に作り替える
2. メニュー提案カードと配送選択肢カードに推薦理由バッジを表示する
3. フィードバック入力 → 再提案フローを実装する
4. トレースパネル・チャット UI を order ページから削除する（support ページは維持）
5. `npm run build` で全ページビルドを確認する
