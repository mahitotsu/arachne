...
- [ ] 優先プロパティ群が `@ConfigurationProperties` へ集約され、サービス設定が `@Value` の散在に依存しない（対応: Phase 5-A）
- [ ] PostgreSQL を使う主要永続化スライスが Spring Data JDBC ベースへ移行し、Spring らしい aggregate / repository 境界で読める（対応: Phase 5-B）
...
**完了基準:**
- [ ] 上記主要サービスで `@Value` の散在が大幅に減り、設定注入の入口が `@ConfigurationProperties` へ揃っている
- [ ] registry / security / session / model mode の設定責務が型として読める
- [ ] 不正な URL / service name / required property 欠落を起動時に検出できる
...
**完了基準:**
- [ ] `order-service` の PostgreSQL 永続化が Spring Data JDBC ベースへ移行している
- [ ] `customer-service` の PostgreSQL 永続化が Spring Data JDBC ベースへ移行している
- [ ] 既存 API 契約と seed 動作が変わらず、主要 API test が回帰通過する
- [ ] `mvn -f food-delivery-demo/pom.xml test` が通る
...
| Phase 5-A: Spring Boot Configuration Properties への設定集約 | ⏳ 未着手 |
| Phase 5-B: Spring Data JDBC への主要永続化スライス移行 | ⏳ 未着手 |
| Phase 5-C: Micrometer Observation と Arachne metrics の可視化 | ⏳ 未着手 |

## Next Action

**次の主作業は Phase 5-A に入り、設定を `@ConfigurationProperties` へ寄せる。その後に Spring Data JDBC と Observation を積む。**

次アクション:
1. `order-service` と `customer-service` から `@ConfigurationProperties` 導入を始め、`registry` / `security` / `session` / `model` の設定境界を型へ切り出す
2. その型付き設定を前提に、`order-service` と `customer-service` の PostgreSQL 永続化を Spring Data JDBC 化する
3. Arachne の `AgentResult.metrics()` と Spring の observation-only event bridge を使った Micrometer 指標化の最小 slice を `order-service` で検証する
4. Phase 5 の 1 slice が落ち着いた時点で、あらためて `make up` と E2E 確認を回して最終完了基準も並行して詰める
