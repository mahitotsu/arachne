...
2. その後に discover_eta_services を呼ぶ。このとき query には「外部 ETA を提供するサービス」など能力を説明する固定フレーズを使い、注文内容のテキストをそのまま渡してはいけません。返ってきた AVAILABLE な候補だけを対象に call_eta_service を実行します。
