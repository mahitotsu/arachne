...
Use [concept.md](/home/akring/arachne/marketplace-agent-platform/docs/concept.md) to describe what the sample is meant to demonstrate.
...
The preferred local shape is one PostgreSQL engine with service-separated schemas or logical databases:
...
The target shared session store for the sample is Redis.
...
- the intended availability slice uses Redis to back shared workflow session continuity
- the composed runtime uses Redis to back shared workflow session continuity across workflow-service replicas
...
For this sample, the shared session persistence layer is Redis and it is part of the composed runtime rather than an optional later substitution.

The sample does not need to simulate instance failure to prove the point; it is enough to show that the load-balanced path works correctly across workflow-service replicas.
...
The initial API surface and contract direction are described in [apis.md](/home/akring/arachne/marketplace-agent-platform/docs/apis.md).
...
- one PostgreSQL engine for business data persistence
