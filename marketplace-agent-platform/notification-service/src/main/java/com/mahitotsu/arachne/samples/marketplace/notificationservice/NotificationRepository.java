...
class NotificationRepository {
    ...
    NotificationDispatchRecord recordDispatch(NotificationContracts.NotificationDispatchCommand command) {
        var existing = findBySettlementReference(command.settlementReference());
        if (existing.isPresent()) {
            return existing.orElseThrow();
        }
        var summary = "Notification service queued participant and operator notifications for settlement reference "
                + command.settlementReference() + ".";
        var now = OffsetDateTime.now(ZoneOffset.UTC);
        jdbcTemplate.update(
                "insert into notification_dispatch (dispatch_id, case_id, outcome_type, outcome_status, settlement_reference, dispatch_status, delivery_status, summary, created_at) values (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                command.settlementReference(),
                command.caseId(),
                command.outcomeType(),
