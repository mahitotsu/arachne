...
| Task | Status |
|---|---|
| Streaming invocation API — 応答を逐次購読できる | `[ ]` |
| Steering Handler — context-aware guidance を plugin として登録できる | `[ ]` |
| Tool steering — tool 実行前の proceed / guide / interrupt を制御できる | `[ ]` |
| Model steering — model 応答後の guidance 付き retry を制御できる | `[ ]` |
