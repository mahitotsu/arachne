...
### Phase 6 — Streaming & Steering `[x]`

**Goal**: 既存の同期 agent runtime を維持したまま、ストリーミング出力と context-aware な実行制御を追加する。

| Task | Status |
|---|---|
| Streaming invocation API — 応答を逐次購読できる | `[x]` |
| Steering Handler — context-aware guidance を plugin として登録できる | `[x]` |
| Tool steering — tool 実行前の proceed / guide / interrupt を制御できる | `[x]` |
| Model steering — model 応答後の guidance 付き retry を制御できる | `[x]` |
| Spring integration — `AgentFactory` から streaming / steering を opt-in で利用可能にする | `[x]` |
