...
| `AgentSkillsPlugin` — skill catalog / activation 経路を hook/plugin 境界の上に載せる | `[ ]` |
| Spring classpath スキャン — `resources/skills/` 配下の SKILL.md を自動検出 | `[x]` |
| `AgentFactory.builder().skills(...)` API | `[x]` |
| Dedicated skill activation tool — available skill catalog を見せ、必要な skill 本文だけを遅延ロードする | `[ ]` |
| 会話中にロード済み skill の重複注入抑止と context management 方針を定める | `[ ]` |
