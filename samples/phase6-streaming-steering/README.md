# Phase 6 Streaming And Steering Sample

This sample is a runnable, Bedrock-free demo for the Phase 6 feature set.
...
The sample uses a deterministic in-process `StreamingModel`, so you can verify the Phase 6 flow without AWS credentials.
...
```bash
cd samples/phase6-streaming-steering
mvn spring-boot:run
```
...
```text
Arachne Phase 6 streaming and steering sample
...
- `SampleStreamingSteeringRunner`: builds the runtime, subscribes to stream events, and prints the final result
...
- `DemoSteeringHandler`: applies tool guidance and guided model retry through the Phase 6 steering contract
...
This intentionally shows the current Phase 6 boundaries.
...
spring:
  application:
    name: arachne-phase6-streaming-steering
  main:
    banner-mode: off
