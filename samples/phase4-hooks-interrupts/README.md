# Phase 4 Hooks And Interrupts Sample

This sample is a runnable, Bedrock-free demo for the Phase 4 feature set.
...
```bash
cd samples/phase4-hooks-interrupts
mvn spring-boot:run
```
...
```text
Arachne Phase 4 hooks and interrupts sample
...
state.workflow> phase4-hooks-demo
...
- `SampleApprovalRunner`: owns the runtime, triggers the first invocation, and resumes it after an interrupt
...
This intentionally shows the current Phase 4 boundaries.
...
spring:
  application:
    name: arachne-phase4-hooks-interrupts
  main:
