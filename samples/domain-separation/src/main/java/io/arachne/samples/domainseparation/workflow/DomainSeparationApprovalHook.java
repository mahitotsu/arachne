...
public class DomainSeparationApprovalHook implements HookProvider {
    ...
    private void captureWorkflowTurn(BeforeInvocationEvent event) {
        ...
        if (isResume(prompt)) {
            boolean approved = Boolean.parseBoolean(prompt.getOrDefault("approvalApproved", "false"));
            log.info(
            ...
            event.state().put(DomainSeparationWorkflowState.STATUS, approved ? "RUNNING" : "REJECTED");
        }
    }
