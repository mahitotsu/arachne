...
この構成は「同じモデル設定を共有しうる」ことを示しますが、サービス境界の有無はこれだけでは決まりません。境界は呼び出し経路で確認する必要があります。

## order-service 側の LLM 呼び出しは service-local（ArachneOrderIntentPlanner.java）

`order-service` は `order-intake-agent` の system prompt と structured output 型を自サービス内で定義し、LLM 呼び出しを実行します。
...
この呼び出しは `NormalizedOrderIntent.class` に閉じた形で実行され、menu-service の型や tool 定義に直接依存しません。
...
この時点で境界は LLM ではなく HTTP 契約です。`order-service` は `MenuSuggestionRequest` を送信し、`MenuSuggestionResponse` を受信します。

## menu-service 側の LLM 呼び出しも service-local（MenuApplicationService.java）

`menu-service` は別の system prompt と別の tool セットを使って、独立に LLM を呼び出します。
...
ここでも出力型は `MenuSelectionDecision.class` であり、order-service の `NormalizedOrderIntent` とは別契約です。

## 観察された振る舞い

- order-service の責務は「注文意図の正規化」で、menu-service の責務は「catalog grounding と候補選定」に分離されていた。
- 両サービスは同じモデル設定を共有しうるが、system prompt・tool・structured output 型はサービスごとに独立して定義されていた。
- サービス間の受け渡しは `MenuSuggestionRequest` の HTTP ペイロードであり、LLM の内部状態を共有する経路はコード上に現れなかった。
