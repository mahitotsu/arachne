...
`requiredInputs`・`optionalInputs`・`serviceBehavior` という構造で、人間にも機械にも読める形でエージェント向けの期待値が API 上に公開されています。
...
この `x-ai-prompt-contract` は、`registry-service` に登録された service descriptor にも含まれており、`/agents` ページが registry から取得して表示します。エージェントのツール定義と REST API 仕様は同一の情報源として管理されています。
...
`agent` フィールドが `"payment-service"` であることに注目してください。`"payment-agent"` ではなく、サービス名そのものが入っています。これは payment-service がエージェントを持たない決定論的なサービスであることを示しています（obs-03 参照）。
