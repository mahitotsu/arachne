...
「テリヤキバーガーください」に `DIRECT_ITEM` を付与するかどうかは LLM が判断します（`plan()` の `agentFactory.builder().run()` の部分）。Java コードはその値を検証して通過させるだけです（`normalizeIntentMode()` の検証ロジック）。また、再注文コンテキストが必要かどうかの判定も LLM に委ねるため、`recentOrder` は常に order-intake-agent に渡されます。
...
`menu-agent` の system prompt では、exact match が弱い場合でも Java コード側で代替提案の根拠を明示するよう指示が含まれています。この指示は、AI が「勝手に選んだ」とユーザーに見えないための設計上のコミットメントです。
...
AI の代替提案が「説明できる根拠」と紐付いていることで、推薦結果の透明性が保たれます。
